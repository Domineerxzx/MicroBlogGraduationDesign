...
public class HotAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (userInfo.getUserHead() != null || userInfo.getUserHead().length() != 0) {
            File file = new File(userInfo.getUserHead());
            boolean exists = file.exists();
            if (file.length() > 0) {
                Glide.with(context).load(file).apply(RequestOptions.bitmapTransform(new CircleCrop())).into(viewHolder.iv_user_head);
            } else {
                //OssHandler ossHandler = new OssHandler(context, viewHolder.iv_user_head, true);
                //DownloadUtils.downloadFileFromOss(file, ossHandler, ProjectProperties.BUCKET_NAME, "MicroBlog/" + userInfo.getUserHead());
            }
        }
        ...
        PhotoWallAdapter photoWallAdapter = new PhotoWallAdapter(context, issueImageInfoList.get(position));
